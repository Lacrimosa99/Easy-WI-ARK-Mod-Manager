...
UPDATER_INSTALL() {
	...
	if [ ! -f /etc/cron.d/ark_mod_updater ]; then
		echo '0 */1 * * * root /root/ark_mod_updater.sh >/dev/null 2>&1' > /etc/cron.d/ark_mod_updater
...
MOD_NAME_CHECK() {
	ARK_MOD_NAME_NORMAL=$(curl -s "https://steamcommunity.com/sharedfiles/filedetails/?id=$MODID" | sed -n 's|^.*<div class="workshopItemTitle">\([^<]*\)</div>.*|\1|p' | tr -d "\t,';=")
